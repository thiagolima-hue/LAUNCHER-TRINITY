...
async function showMainUI(data) {
    console.log('DistroUI: Ensuring Java settings...')
    ...
    console.log('DistroUI: Preparing settings...')
    await prepareSettings(true)
    console.log('DistroUI: Updating selected server...')
    ...
    console.log('DistroUI: Refreshing server status...')
    refreshServerStatus()
    setTimeout(() => {
        document.getElementById('frameBar').style.backgroundColor = '#7fc734'
...
// Race condition fix: check if we already have the distro loaded
console.log('DistroUI: Checking for existing distribution...')
DistroAPI.getDistribution().then(data => {
    if (data) {
        console.log('DistroUI: Distribution found on load, triggering UI...')
        if (document.readyState === 'interactive' || document.readyState === 'complete') {
            showMainUI(data)
        } else {
            rscShouldLoad = true
    ...
    console.log('DistroUI: No distribution found on initial load.', err)
})
